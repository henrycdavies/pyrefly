...
### Custom `.pth` File Directories

Pyrefly automatically expands `.pth` files found in manually configured `site-package-path` directories to align 
behavior with interpreter-queried site-packages. When you set `site-package-path = ['./python_path']` in your 
configuration, Pyrefly will scan for `.pth` files in that directory and add the paths they reference to the import 
search path, mimicking Python's standard `.pth` file processing behavior. See the [`site-package-path` configuration 
option](configuration.mdx#site-package-path) for more details.
