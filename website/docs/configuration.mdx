...
- Notes:
  ...
  - **`.pth` file expansion**: Pyrefly automatically expands `.pth` files found in manually configured
    `site-package-path` directories to align behavior with interpreter-queried site-packages. Only 
    path-based `.pth` entries are supported; lines starting with `import` (import hooks) are ignored 
    as they require runtime code execution. When using [Environment Autoconfiguration](#environment-autoconfiguration), 
    Python has already expanded `.pth` files in its standard site-packages directories, so this expansion 
    only applies to manually configured paths.
